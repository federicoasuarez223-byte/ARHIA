...
import { fail, created, ok, paginatedOk } from '@/lib/response';
...
export async function handleUpdateReview(req: TenantRequest, res: Response) {
  if (!req.companyId) return fail(res, 401, 'UNAUTHORIZED', 'No autenticado');
  ...
  try {
    const review = await PerformanceService.updateReview(req.companyId, req.params.id, parsed.data);
    ok(res, review);
  } catch (err) {
    handleError(res, err);
  }
